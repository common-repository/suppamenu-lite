...
(function($) {
    ...
    var suppa_menu = {
        ...
        eventOnClickMenuSave : function() {
            ...
            // Suppamenu Part
            if( suppa_menu.suppaAjaxSaved === false ){

	            var $suppaItems = {};
	            api.menuList.find('.suppa_menu_item').each(function(){

	                var $self = $(this);
	                var $id = $self.attr('id').split('-item-')[1];
	                $suppaItems[ $id ] = {};

	                $self.find(':input[name*="suppa"]').each(function(){
	                    $suppaItems[ $id ][ '_' + $(this).attr('name').replace('['+$id+']', '') ] = $(this).val();
	                    $(this).remove();
	                });
        ...
        fullwidth_checkbox_click : function() {
            jQuery( document ).on('click', '.admin_suppa_fullwidth_checkbox', function(){
                var $this = jQuery(this);

                if( $this.is(':checked') ) {
                    $this.val('on')
                    	.attr('checked','checked')
                    	.parents('.admin_suppa_box_option_inside')
                    	.find('.admin_suppa_fullwidth_div')
                    	.hide();
                }
                else {
                	$this.val('off')
        ...
        fullwidth_checkbox : function(){
            jQuery('.admin_suppa_fullwidth_checkbox').each(function(){
                var $this = jQuery(this);
                if( $this.val() == "on" ){
                    $this.parents('.admin_suppa_box_option_inside')
                    	.find('.admin_suppa_fullwidth_div')
                    	.hide();
                }
                else{
